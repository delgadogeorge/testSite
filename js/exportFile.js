//This script takes modified data and filters it into 3 categories
//The data is then exported as an excel file
...
function exportFile(jsonMod) {
	...
	jsonMod.forEach((row) => {
		...
		if (
			doorKeyWords.some((keyWord) => eventType.includes(keyWord.toLowerCase()))
		) {
		...
		if (
			nullKeyWords.some((keyWord) => source.includes(keyWord.toLowerCase()))
		) {
